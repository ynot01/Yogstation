...
//this is very slightly better than it was because you can use it more places. still can't do \his[src] though.
	...
	if(ishuman(user))
		...
		for(var/datum/reagent/R in reagents.reagent_list)
			if(R.hidden)
				hidden_volume += R.volume
		if(H.can_see_reagents() && (reagents?.total_volume - hidden_volume) > 0)
			. += "Reagents detected: [reagents.total_volume - hidden_volume]u of [LAZYLEN(reagents.reagent_list)] chemicals."
